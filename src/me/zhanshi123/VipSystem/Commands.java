...
public class Commands implements CommandExecutor {
	...
	if (cmd.getName().equalsIgnoreCase("vipsys")) {
		...
		if (args[0].equalsIgnoreCase("give") && sender.isOp()) {
			...
			if (Main.getDataBase().getGroup(name).equals(group)
				|| Main.getDataBase().getGroup(name).equals("0")) {
			    Utils.addVip(name, group, String.valueOf(time));
			    Main.getPlaceholderCache().flushData(name);
			    sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
				    MessageManager.prefix + MessageManager.VipGave));
			    return true;
			} else {
			    sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
				    MessageManager.prefix + MessageManager.AlreadyHaveVip));
			    return false;
			}
		    } else {
			Utils.addVip(name, group, String.valueOf(time));
			...
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
				MessageManager.prefix + MessageManager.PlayerNotFound));
			return false;
		    }
		    if (Main.getDataBase().getExpired(name) == 0) {
			Utils.removeVip(p.getName());
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
				MessageManager.prefix + MessageManager.VipRemoved));
		    } else {
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
				MessageManager.prefix + MessageManager.HaveNoVip));
		    }
			...
			if (!left.equals("-1")) {
			    ...
			    sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
				    MessageManager.prefix + MessageManager.QueriedByAdmin
					    .replace("%player%", p.getName()).replace("%group%", group)
					    .replace("%date%", dates).replace("%left%", String.valueOf(days))));
			} else {
			    sender.sendMessage((MessageManager.prefix + MessageManager.QueriedByAdminPermanent)
				    .replace("%group%", group).replace("&", "¡ì").replace("%player%", p.getName()));
			}
		    } else {
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
				MessageManager.prefix + MessageManager.HaveNoVip));
		    ...
		    sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
			    MessageManager.prefix + MessageManager.CacheSaved));
			...
			if (Main.getDataBase().getExpired(name) == 0) {
				...
				sender.sendMessage(ChatColor
					.translateAlternateColorCodes('&',
						MessageManager.prefix + MessageManager.QueriedBySelf)
					.replace("%group%", group).replace("%left%", String.valueOf(days))
					.replace("%date%", dates));
			    } else {
				sender.sendMessage((MessageManager.prefix + MessageManager.QueriedBySelfPermanent)
					.replace("&", "¡ì").replace("%group%", group));
			    }
			} else {
			    sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
				    MessageManager.prefix + MessageManager.YouHaveNoVip));
			}
		    }
			...
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
				MessageManager.prefix + MessageManager.ConsoleForbid));
			return true;
		    }
		    String name = Utils.getPlayerName(p);
		    String cdk = args[1];
			...
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
				MessageManager.prefix + MessageManager.CodeNotFound));
		    } else {
			...
			if (Main.getDataBase().exists(name)) {
				...
				sender.sendMessage(ChatColor
					.translateAlternateColorCodes('&',
						MessageManager.prefix + MessageManager.CodeActivated)
					.replace("%vip%", key.getGroup()).replace("%left%", days));
			    } else {
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					MessageManager.prefix + MessageManager.YouAlreadyHaveVip));
			    }
			} else {
		    ...
		    sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
			    MessageManager.prefix + MessageManager.ThreadCreated));
		} else if (args[0].equalsIgnoreCase("export") && sender.isOp()) {
		    String group = args[1];
		    long start = System.currentTimeMillis();
		    List<String> list = Main.getDataBase().getKeys(group);
		    File f = new File(Main.getInstance().getDataFolder(),
			    "Keys-" + group + "-" + System.currentTimeMillis() + ".txt");
		    try {
			...
			sender.sendMessage(ChatColor
				.translateAlternateColorCodes('&', MessageManager.prefix + MessageManager.CodeExport)
				.replace("%time%", String.valueOf(end - start)).replace("%path%", f.getAbsolutePath()));
		    } catch (Exception e) {
			sender.sendMessage(ChatColor
				.translateAlternateColorCodes('&',
					MessageManager.prefix + MessageManager.CodeExportFailed)
